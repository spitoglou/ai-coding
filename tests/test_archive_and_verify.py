"""Tests for archive_reports.py and verify.py/verify.sh."""
...
def _verify_sh(project: Path, *args: str):
    return subprocess.run(
        ["bash", ".claude/skills/agent-coordination/scripts/verify.sh", *args],
        cwd=project, capture_output=True, text=True,
    )


def test_verify_sh_runs_all_checks_without_aborting(project: Path):
    # Regression guard: set -e + ((PASS++)) used to abort after the first PASS.
    add_dated_report(project, "review", "v", "2026-07-24")
    r = _verify_sh(project, "review", "v", "2026-07-24")
    assert r.stdout.count("Report file exists") == 1
    assert "Report has content" in r.stdout
    assert "Registry entry exists" in r.stdout


def test_verify_sh_passes_for_valid_deliverable(project: Path):
    add_dated_report(project, "review", "v", "2026-07-24")
    r = _verify_sh(project, "review", "v", "2026-07-24")
